...
class Encoder(nn.Module):
	...
	def __init__(self, opt, dicts):
		...
		input_size = opt.word_vec_size_enc

		super(Encoder, self).__init__()
		self.word_lut = nn.Embedding(dicts.size(),
								  opt.word_vec_size_enc,
...
class Decoder(nn.Module):
	...
	def __init__(self, opt, dicts):
		...
		input_size = opt.word_vec_size_dec
		if self.input_feed:
			input_size += opt.out_size

		super(Decoder, self).__init__()
		self.word_lut = nn.Embedding(dicts.size(),
								  opt.word_vec_size_dec,
								  padding_idx=onmt.Constants.PAD)
		self.rnn = StackedLSTM(opt.layers, input_size, opt.rnn_size, opt.dropout)
		self.attn = onmt.modules.GlobalAttention(opt.rnn_size, opt.out_size)
...
class NMTModel(nn.Module):
	...
	def make_init_decoder_output(self, context):
		...
		h_size = (batch_size, self.decoder.out_size)
